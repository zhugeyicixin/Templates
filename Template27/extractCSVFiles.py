...
pattern_folder = re.compile('^Factor0.5_([0-9]+)$')
...
if os.path.exists(pwd_home + '/CSVFileCollection'):
	shutil.rmtree('CSVFileCollection')
os.mkdir('CSVFileCollection')
	...
	if tmp_m:
		...
		for tmp_file in tmp_fileLists:
			...
			if re.search('.csv',tmp_file):
				...
				# 	error_file_num += 1
				shutil.copyfile(tmp_file, pwd_home + '/CSVFileCollection/' + tmp_file[0:-8] + '%04d'%((int(tmp_m.group(1))-1)*20+int(tmp_file[-8:-4])) + '.csv')
		os.chdir('../')
