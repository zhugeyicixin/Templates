	...
	if re.search(name,tmp_file):		
		...
		for tmp_file2 in tmp_fileLists2:
			...
			if tmp_m:
				...
				for i in range(0,len(tmp_lines)):
					...
					if multi_done != 1:
						...
						if tmp_m:
							geom_start = i + 1
					if rigidScan_done != 1:
						tmp_m = pattern_rigidScan.match(tmp_line)
						if tmp_m:
							steps = int(tmp_m.group(3))
							step_length = float(tmp_m.group(4))
							dihedral.append(float(tmp_m.group(2)))
							pattern_dihedral = re.compile('^ *[a-zA-Z0-9]+ *([0-9]+) *[0-9]+\.[0-9]* *([0-9]+) *[0-9]+\.[0-9]* *([0-9]+) *' + tmp_m.group(1) + ' *0.*$')
							for j in range(geom_start, i):
								tmp2_line = tmp_lines[j]
								tmp2_m = pattern_dihedral.match(tmp2_line)
								if tmp2_m:
									atoms = [j - geom_start + 1, int(tmp2_m.group(1)), int(tmp2_m.group(2)), int(tmp2_m.group(3))]
							rigidScan_done = 1
					elif coordinate_done != 1:
						if coordinate_done == -1:
							tmp_m = pattern_Zcoordinate.match(tmp_line)
							if tmp_m:
								geom_start = i + 5
								coordinate_done = 0
						elif coordinate_done == 0:
						...
						if tmp_m:
							...
							if len(energy) > 1:
								dihedral.append(dihedral[-1] + step_length)
							coordinate_done = -1
							energy_done = 1
				...
				if not tmp_m:
					print 'Warning! Notice that job ' + tmp_file2 + ' did not end successfully!'
				# else:
