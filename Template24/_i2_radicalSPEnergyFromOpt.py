...
# extract energies
if os.path.exists('_i2_radicalSPEnergy'):
	shutil.rmtree('_i2_radicalSPEnergy')
os.mkdir('_i2_radicalSPEnergy')
...
for tmp_mole in  molecules:
	...
	# if len(sortedDict) > 0:
	# 	shutil.copyfile(os.path.join(pwd, tmp_folder, sortedDict[0][0]+'.log'), os.path.join(pwd, '_i2_radicalSPEnergy', sortedDict[0][0]+'.log'))
	# else:
			...
			if multi_done != 1:
				...
				if tmp_m:
					if lineNum > tmp_num:
						...
						coordinate_done = 1
		fw = file(os.path.join('..', '_i2_radicalSPEnergy', tmp_file[0]+'.gjf'), 'w')
		fw.write(
'''%mem=28GB
%nprocshared=12
%chk=''' + tmp_file[0] + '''.chk
...
# generate B3LYP jobs from Gjfs in _i2_radicalSPEnergy
os.chdir('_i2_radicalSPEnergy')
...
for tmp_file in tmp_fileList:
	if re.search('\.gjf', tmp_file):
		...
		if tmp_m:
			cluster1.generateJobFromGjf(tmp_file, jobName=tmp_m.group(1)+'_9_SP_M06D3' ,command='#p M062X/def2TZVP EmpiricalDispersion=GD3')
