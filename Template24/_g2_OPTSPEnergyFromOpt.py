...
clusterName = 'Tianhe2'
clusterPath = '/vol-th/home/you1/hetanjin/newGroupAdditivityFrog2/_g2_CnH2n_5'
jobsPerSlot = 10
...
# extract energies
if os.path.exists('_g2_OPTSPEnergy'):
	shutil.rmtree('_g2_OPTSPEnergy')
os.mkdir('_g2_OPTSPEnergy')
...
for tmp_mole in  molecules:
	...
	# if len(sortedDict) > 0:
	# 	shutil.copyfile(os.path.join(pwd, tmp_folder, sortedDict[0][0]+'.log'), os.path.join(pwd, '_g2_OPTSPEnergy', sortedDict[0][0]+'.log'))
	# else:
			...
			if multi_done != 1:
				...
				if tmp_m:
					if lineNum > tmp_num:
						...
						coordinate_done = 1
		fw = file(os.path.join('..', '_g2_OPTSPEnergy', tmp_file[0]+'.gjf'), 'w')
		fw.write(
'''%mem=28GB
%nprocshared=12
%chk=''' + tmp_file[0] + '''.chk
...
# generate B3LYP jobs from Gjfs in _g2_OPTSPEnergy
os.chdir('_g2_OPTSPEnergy')
...
for tmp_file in tmp_fileList:
	if re.search('\.gjf', tmp_file):
		...
		if tmp_m:
			cluster1.generateJobFromGjf(tmp_file, jobName=tmp_m.group(1)+'_4_opt_M06' ,command='#p M062X/def2TZVP opt freq')
...
print '\nLog of task 5\n'
print 'Cluster script generated successfully!'
# THE END
