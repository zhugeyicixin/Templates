...
for (index, tmp_name) in enumerate(reacNames):
	...
	tmp_coeff, tmp_deviation = arrhenius.fit_arrhenius_noGuess(T_fit, rate_f[index][2:13], threshold=1e-2)
	coeff_f.append(tmp_coeff)
	deviation_f.append(tmp_deviation)
	tmp_coeff, tmp_deviation = arrhenius.fit_arrhenius_noGuess(T_fit, rate_r[index][2:13], threshold=1e-2)
	coeff_r.append(tmp_coeff)
	deviation_r.append(tmp_deviation)
	tmp_coeff, tmp_deviation = arrhenius.fit_arrhenius_noGuess(T_fit, Kconst[index][2:13], threshold=1e-2)
...
tmp_fig2.show()
tmp_fig3.show()
tmp_fig.savefig('rate_forward' + '.png',dpi=300)
tmp_fig2.savefig('rate_reverse' + '.png',dpi=300)
tmp_fig3.savefig('equilibrium constants' + '.png',dpi=300)
plt.close(tmp_fig)
plt.close(tmp_fig2)
plt.close(tmp_fig3)
